...
(let* ((global (make-instance 'basic-context :address #x8000))
  ...
  (procedure reset
    ...
    (with-label mainloop
      ...
      (lda (mem +ppu-status+))          ; Set scroll register

      (ldx phase)
      (inx)
      (stx phase)
      (lda scroll-x)                    ; Scroll horizontally..
      (clc)                             ; Increment scroll-x
;      (adc (imm 1))
      (adc (abx 'rate-pattern))
      ...
      (sta ntaddr)
      (ora (imm #b10001000))            ; NMI on, BG Patterns $0000
      (sta (mem +ppu-cr1+))
      ...
      (lda ntaddr)
      (ora (imm #b10011000))            ; NMI on, BG Patterns $1000
      (sta (mem +ppu-cr1+))
  ...
  (with-label rate-pattern
    (let* ((tr '(1 0 0 0 1 0 0 1 0 1 0 1 01))
           (motion (concatenate 'list
                                (loop repeat (- 128 (reduce '+ tr))
                                      collect 1)
                                (reverse tr)))
           (pattern (concatenate 'list
                                 motion
                                 (loop repeat (- 256 (length motion))
                                       collect 0))))
