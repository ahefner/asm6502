  ...
  ;;;; -------------------------------------------------------
    ...
    (subprogram (ram-test "RAM Test 1")
      (let* ((test-mode (zp 2))
        ...
        (ldx (imm 255))
        (jsr (mem (label 'fill-nametable-x)))
        (pokeword (label :ram-test-string :offset -1) addr)
        (lda (mem +ppu-status+))
        ...
        (with-label :testing
          ...
          (asif :zero (pokeword (label :pass :offset -1) addr)
                :else (pokeword (label :fail :offset -1) addr))
          (ppuaddr #x20CE)
          (ldy (imm 4))
          (as/until :zero
            (poke (indi addr) +vram-io+)
...
;;; Real interrupt handlers
   ...
   ;;; 9/10: Pointer to first menu item
    ...
    (inx)
    (asif :zero
      (ldx (mem (label 'menu-bypass :offset 1)))
      (inx)
